...
import zlib
import http.cookiejar as cookielib  # 用于生成cookie
import time


class Http(object):
    @staticmethod
    def get_content(url='', data=None, timeout=3, extra_header={}):
        u"""获取网页内容
        ...
        try:
            ...
            print(u'网页打开失败')
            print(u'失败页面:{}'.format(url))
            print(u'失败代码:{}'.format(error.code))
            print(u'失败原因:{}'.format(error.reason))
        except urllib2.URLError as error:
            print(u'网络连接异常')
            print(u'异常页面:{}'.format(url))
            print(u'异常原因:{}'.format(error.reason))
        except socket.timeout as error:
            print(u'打开网页超时')
            print(u'超时页面:{}'.format(url))
        except socket.error:
            print(u'打开网页超时')
            print(u'超时页面:{}'.format(url))
        except Exception:
            print(u'未知错误')
            print(u'错误页面:{}'.format(url))
            print(u'错误堆栈信息:{}'.format(traceback.format_exc()))
        else:
